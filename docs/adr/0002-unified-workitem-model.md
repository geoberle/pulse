...
**TypeMeta + Spec as json.RawMessage** (chosen) — common fields are always available for rendering and diffing. Type-specific data lives in Spec, unmarshaled on demand into typed structs (JiraSpec, PRSpec, ReviewSpec, CheckSpec). Adding a new kind means adding a new Spec struct and a case in the unmarshal switch — the tree, renderer, informer, and persistence layer don't change.
...
- IDs follow `{source}:{identifier}` pattern, making them self-documenting and collision-free across kinds.
